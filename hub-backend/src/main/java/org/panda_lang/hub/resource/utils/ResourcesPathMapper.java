...
package org.panda_lang.hub.resource.utils;

import org.panda_lang.hub.HubApplication;
import org.panda_lang.hub.resource.Resource;
import org.panda_lang.hub.resource.ResourcesSubService;
...
public final class ResourcesPathMapper<R extends Resource<R>> {
    ...
    public Optional<R> map(String qualifier) {
        ...
        if (elements.length < path.getElements().size()) {
            HubApplication.getLogger().warn("Skipping " + qualifier);
        ...
        for (int index = 0; index < elements.length; index++) {
            ...
            if (unit.isStatic()) {
                ...
                HubApplication.getLogger().warn("Unknown element: " + qualifier + " (" + value + ")");
                return Optional.empty();
            }
